...
psp_lm <- function(dv, iv){
  ...
  lm1 <- estimatr::lm_robust(get(dv) ~ get(iv) + as.factor(country), 
                             data = psp, se_type = "CR2", clusters = country)
  lm2 <- estimatr::lm_robust(get(dv) ~ get(iv) + as.factor(country) + as.factor(year), 
                             data = psp, se_type = "CR2", clusters = country)
  lm3 <- estimatr::lm_robust(get(dv) ~ get(iv) + as.factor(country) + as.factor(year) + qog_wbgi_pve, 
                             data = psp, se_type = "CR2", clusters = country)
  lm4 <- estimatr::lm_robust(get(dv) ~ get(iv) + as.factor(country) + as.factor(year) + qog_wbgi_pve + qog_wdi_gini, 
                             data = psp, se_type = "CR2", clusters = country)
  lm5 <- estimatr::lm_robust(get(dv) ~ get(iv) + as.factor(country) + as.factor(year) + qog_wbgi_pve + qog_wdi_gini + qog_gle_pop, 
                             data = psp, se_type = "CR2", clusters = country)
  lm6 <- estimatr::lm_robust(get(dv) ~ (get(iv) * tb_other_provis) + as.factor(country) + as.factor(year) + qog_wbgi_pve + qog_wdi_gini + qog_gle_pop, 
                             data = psp, se_type = "CR2", clusters = country)
  ...
  lm_full_tex <- texreg::texreg(list(lm1, lm2, lm3, lm4, lm5, lm6), 
                                mfrow = TRUE, 
                                omit.coef = "as.factor", 
                                include.ci = FALSE, 
                                custom.gof.rows = list(`Country fixed effects` = c("Y", "Y", "Y", "Y", "Y", "Y"),
                                                        `Year fixed effects` = c("N", "Y", "Y", "Y", "Y", "Y")))
