...
reg_data_complete <- reg_data[complete.cases(reg_data), ] %>%
  dplyr::select(cown, year, dpi_auton, n_RAI, reg_aut_dum, idc_subtax, idc_subed, idc_subpolice, dpi_author, polity4_polity_score)

# correlation table
auton_cor_pear <- xtable::xtable(round(cor(reg_data_complete[,3:9]),2))
...
auton_cor_spear <- xtable::xtable(round(cor(reg_data_complete[,3:9], method = "spearman"),2))
auton_cor_spear
auton_cor_ken <- xtable::xtable(round(cor(reg_data_complete[,3:9], method = "kendall"),2))
auton_cor_ken
...
plot(x = reg_data_complete$n_RAI, y = reg_data_complete$polity4_polity_score,
     xlab = "RAI Measure of Autonomy", ylab = "Polity Score", main = "Predicting Polity Score with Regional Autonomy Provisions")
