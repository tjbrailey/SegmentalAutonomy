...
# Some values were not matched unambiguously: Republic of Vietnam, Würtemberg
...
wvs_cleaned$A035[wvs_cleaned$A035 == 0] <- 2
wvs_cleaned$A035[wvs_cleaned$A035 == 1] <- 0
wvs_cleaned$A035[wvs_cleaned$A035 == 2] <- 1

wvs_cleaned[wvs_cleaned == -5] <- NA
wvs_cleaned[wvs_cleaned == -4] <- NA
wvs_cleaned[wvs_cleaned == -3] <- NA
wvs_cleaned[wvs_cleaned == -2] <- NA
wvs_cleaned[wvs_cleaned == -1] <- NA
...
wvs_psp_sub <- wvs_cleaned %>%
  ...
  dplyr::select(S003, S020, A035, A124_avg)

wvs_psp_sub$A035[is.nan(wvs_psp_sub$A035)] <- NA
wvs_psp_sub$A124_avg[is.nan(wvs_psp_sub$A124_avg)] <- NA

wvs_psp_sub <- wvs_psp_sub %>%
  dplyr::group_by(S003, S020) %>%
...
wvs_psp_sub$country <- countrycode::countrycode(wvs_psp_sub$S003, "wvs", "country.name")
wvs_psp_sub$cowc <- countrycode::countrycode(wvs_psp_sub$S003, "wvs", "cowc")
wvs_psp_sub$cown <- countrycode::countrycode(wvs_psp_sub$S003, "wvs", "cown")

wvs_psp_sub <- wvs_psp_sub %>%
  dplyr::select(-country, -S003)
...
tb_6 <- dplyr::left_join(tb_6, intra_psp_sub, by = c("cown", "cowc", "year"))
tb_6 <- dplyr::left_join(tb_6, p4_psp_sub, by = c("cown", "cowc", "year"))
...
psp_add_bin <- psp_rename %>%
  dplyr::mutate(other_provis = ifelse(idc_mveto == 1|
                                      idc_gcman == 1 |
                                        idc_gcimp == 1 |
                                        dpi_pr == 1, 1, 0),
...
# Factor Analysis
```{r}
wvs_035_124 <- data.frame(psp_add_bin) %>%
  dplyr::select(wvs_A035_child_qualities, wvs_A124_neighbors
                )

cor(wvs_035_124, use = "complete.obs")

pcal_035_124 <- princomp(na.omit(wvs_035_124), scores = TRUE, cor = TRUE)
summary(pcal_035_124)

loadings(pcal_035_124)

plot(pcal_035_124)
screeplot(pcal_035_124, type = "line", main = "Scree Plot")

biplot(pcal_035_124)

x <- data.frame(pcal_035_124$scores)
x$names <- rownames(x)

psp_add_bin$names <- rownames(psp_add_bin)
psp_pca <- dplyr::left_join(psp_add_bin, x) %>%
  dplyr::select(-names, -Comp.2) %>%
