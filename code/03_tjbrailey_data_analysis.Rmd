...
plot1 <- ggplot(wvs_seg_aut, aes(x = n, y = avg)) +
  ...
  geom_smooth() + 
  theme_bw() +
  labs(title = "Factored Public Tolerance by Percentage of Years Under Segmental Autonomy",
...
plot2 <- ggplot(wvs_seg_aut, aes(x = n, y = avg)) +
  ...
  geom_smooth() + 
  theme_bw() +
  labs(title = "Public Tolerance by Percentage of Years Under Segmental Autonomy",
       x = "Percentage of Years Under Segmental Autonomy",
       y = "Factored World Values Survey Response") +
  theme(title = element_text(size = 16),
        axis.title.x = element_text(size = 14),
...
plot3 <- ggplot(wvs_seg_aut, aes(x = n, y = avg)) +
  ...
  geom_smooth() + 
  theme_bw() +
  labs(title = "Tolerance Toward Neighbors by Percentage of Years Under Segmental Autonomy",
       x = "Percentage of Years Under Segmental Autonomy",
       y = "Factored World Values Survey Response") +
  theme(title = element_text(size = 16),
        axis.title.x = element_text(size = 14),
...
satdem_avg <- psp %>%
  dplyr::group_by(country) %>%
  dplyr::filter(any(!is.na(qog_hum_satdem))) %>%
  dplyr::summarize(avg = mean(qog_hum_satdem, na.rm = TRUE))
...
polity_avg <- psp %>%
  dplyr::group_by(country) %>%
  dplyr::filter(any(!is.na(polity4_polity_score))) %>%
  dplyr::summarize(avg = mean(polity4_polity_score, na.rm = TRUE))
...
wvs_lm_subtax_full_tex <- texreg::texreg(list(wvs_lm1_subtax, wvs_lm2_subtax, wvs_lm3_subtax, wvs_lm4_subtax, wvs_lm5_subtax), mfrow = TRUE, omit.coef = "as.factor", include.ci = FALSE, custom.gof.rows = list(`Country fixed effects` = c("Y", "Y", "Y", "Y", "Y"),
                        `Year fixed effects` = c("N", "Y", "Y", "Y", "Y")))
wvs_lm_subtax_full_tex


# Subed
wvs_lm1_subed <- estimatr::lm_robust(wvs_factor ~ idc_subed + as.factor(country), data = psp, se_type = "CR2", clusters = country)
...
wvs_lm_subed_full_tex <- texreg::texreg(list(wvs_lm1_subed, wvs_lm2_subed, wvs_lm3_subed, wvs_lm4_subed, wvs_lm5_subed), mfrow = TRUE, omit.coef = "as.factor", include.ci = FALSE, custom.gof.rows = list(`Country fixed effects` = c("Y", "Y", "Y", "Y", "Y"),
                        `Year fixed effects` = c("N", "Y", "Y", "Y", "Y")))
wvs_lm_subed_full_tex
...
wvs_lm_subpolice_full_tex <- texreg::texreg(list(wvs_lm1_subpolice, wvs_lm2_subpolice, wvs_lm3_subpolice, wvs_lm4_subpolice, wvs_lm5_subpolice), mfrow = TRUE, omit.coef = "as.factor", include.ci = FALSE, custom.gof.rows = list(`Country fixed effects` = c("Y", "Y", "Y", "Y", "Y"),
                        `Year fixed effects` = c("N", "Y", "Y", "Y", "Y")))
wvs_lm_subpolice_full_tex
```

# Linear Regression QOG Democracy Information
```{r}
# Social Trust
trust_lm1 <- estimatr::lm_robust(qog_hum_trust ~ idc_auton + as.factor(country), data = psp, se_type = "HC0")
satdem_lm1 <- estimatr::lm_robust(qog_hum_satdem ~ (idc_auton * other_provis) + as.factor(country), data = psp, se_type = "HC0")
supdem_lm1 <- estimatr::lm_robust(qog_hum_supdem ~ idc_auton + as.factor(country), data = psp, se_type = "HC0")

trust_lm1_tex <- texreg::texreg(trust_lm1, omit.coef = "as.factor", include.ci = FALSE, custom.gof.rows = list(`Fixed effects` = ("Y")))
satdem_lm1_tex <- texreg::texreg(satdem_lm1, omit.coef = "as.factor", include.ci = FALSE, custom.gof.rows = list(`Fixed effects` = ("Y")))
supdem_lm1_tex <- texreg::texreg(supdem_lm1, omit.coef = "as.factor", include.ci = FALSE, custom.gof.rows = list(`Fixed effects` = ("Y")))
